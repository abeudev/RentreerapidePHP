...
class MY_Shop_Controller extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->data['default_colors']    = $this->db->get_where('fronend_color_settings',['id'=>1])->row();
        $this->data['default_style']     = $this->db->get_where('frontend_styles',['id'=>1])->row();
        $this->data['default_font']      = $this->db->get_where('fonts',['id'=>$this->data['default_style']->font_id])->row();
        $this->data['default_font_size'] = $this->com_model->parseArray($this->db->get_where('frontend_font_size',['id'=>'1'])->result(),'font_id' , 'font_size');
    }

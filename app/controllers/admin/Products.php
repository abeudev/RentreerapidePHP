...
//        $warehouses =  $this->site->getWarehouseByID($this->session->userdata('warehouse_id'));
        ...
        $_POST['slug'] = $_POST['slug'].'-'.random_string('alnum',8);
        if (SHOP) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $company_data = $this->db->get_where('companies',['id'=>$this->session->userdata('company_id')])->row();
            $data     = [
                ...
                'supplier1'         => $this->session->userdata('company_id'),
                ...
                'warehouse_name'    => $company_data->name,
...
//                        }
        ...
        if ($this->form_validation->run() == true && $this->products_model->addProduct($data, $items, $warehouse_qty, $product_attributes, $photos)) {
            ...
            $this->data['product_group']       = $this->db->get('shipping_groups')->result();
            $this->data['unit'] = $this->db->get_where('units',['id'=>'1'])->row();
...
//                        }
        ...
        if ($this->form_validation->run() == true && $this->products_model->updateProduct($id, $data, $items, $warehouse_qty, $product_attributes, $photos, $update_variants)) {
           $this->db->where(['id'=>$id])->update('products',['quantity'=>$_POST['quantity']]);
            ...
            $this->data['product_group']       = $this->db->get('shipping_groups')->result();
            $this->data['produc_group_selected']       = $this->db->get_where('product_shipping_group',['product_id'=>$id])->row();
    ...
    public function getadjustments($warehouse_id = null)
    {
        ...
        $delete_link = "<a href='#' class='tip po' title='<b>" . $this->lang->line('delete_adjustment') . "</b>' data-content=\"<p>"
            ...
            . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash\"></i></a>";
    ...
    public function getProducts($warehouse_id = null)
    {
        ...
        $supplier = $this->input->get('supplier') ? $this->input->get('supplier') : $this->session->userdata('company_id');
    ...
    public function import_csv()
    {
            ...
            if (isset($_FILES['userfile'])) {
                ...
                foreach ($arrResult as $key => $value) {
                    ...
                    $item = [
                        ...
                        'brand'             => isset($value[1]) ? trim($value[1]) : '',
                        'category_code'     => isset($value[2]) ? trim($value[2]) : '',
                        'subcategory_code'  => isset($value[3]) ? trim($value[3]) : '',
                        'unit'              => isset($value[4]) ? trim($value[4]) : '',
                        'cost'              => isset($value[5]) ? trim($value[5]) : '',
                        'price'             => isset($value[6]) ? trim($value[6]) : '',
                        'quantity'          => isset($value[7]) ? trim($value[7]) : '',
                        'alert_quantity'    => isset($value[8]) ? trim($value[8]) : '',
                        'hsn_code'          => isset($value[9]) ? trim($value[9]) : '',
                        'second_name'       => isset($value[10]) ? trim($value[10]) : '',
                        'image'             => isset($value[11]) ? trim($value[11]) : '',
                        'product_details'   => isset($value[12]) ? trim($value[12]) : '',
                        ...
                        'supplier1'         => $this->session->userdata('company_id'),
                        'warehouse'         => $this->session->userdata('warehouse_id'),
                        'warehouse_name'    => ucwords($company_data->name),
                        'code'              => random_string('alnum',12),
                        'type'              => 'standard',
                        'barcode_symbology' => 'code128',
                        'sale_unit'         => isset($item['unit']) ? trim($item['unit']) : '',
                        'purchase_unit'     => isset($item['unit']) ? trim($item['unit']) : '',
                        'tax_rate'          => '',
                    ];
        ...
        if ($this->form_validation->run() == true && !empty($items)) {
            ...
            if($added_count >0){
                $this->session->set_flashdata('message', sprintf(lang('products_added'), $added_count) . $updated);
            }
            admin_redirect('products');
        ...
        if ($rows) {
            foreach ($rows as $row) {
                ...
                $pr[]        = [
                    'id' => $c,
                    'item_id' => $row->id,
                    'label' => ''.$row->name . ' (' . $row->warehouse_name . ') : '.$this->sma->formatMoney($this->sma->formatDecimal($row->price)),
                    'row' => $row, 'options' => $options,
                    ];
            }
            $this->sma->send_json($pr);

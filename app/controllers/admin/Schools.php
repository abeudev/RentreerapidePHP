...
class Schools extends MY_Controller
{
    public function __construct()
    {
        ...
        $this->load->admin_model('school_model');
    ...
    public function index()
    {
        ...
        $bc                  = [['link' => base_url(), 'page' => lang('home')], ['link' => '#', 'page' => lang('schools')]];
        $meta                = ['page_title' => lang('schools'), 'bc' => $bc];
    ...
    public function getSchools()
    {
        ...
        $this->datatables
            ->select('id , systm , address , code , name , cycle , phone , dren , order , type')
            ->from('schools')
            ->add_column('Actions', "<div class=\"text-center\">
                        <a href='" . admin_url('schools/edit_school/$1') . "' data-toggle='modal' data-target='#myModal' class='tip' title='" . lang('edit_school')."'>
                            <i class=\"fa fa-edit\"></i>
                        </a>
                        <a href='" . admin_url('schools/classes/$1') . "' data-toggle='modal' data-target='#myModal' class='tip' title='" . lang('orders')."'>
                            <i class=\"fa fa-th-list\"></i>
                        </a>
                        <a href='#' class='tip po' title='<b>" . $this->lang->line(' supprimée l’établissements') . "</b>' data-content=\"<p>" . lang('r_u_sure') . " </p><a class='btn btn-danger po-delete' href='" . admin_url('schools/delete_school/$1') . "'>" . lang('i_m_sure') . " </a> <button class='btn po-close'>" . lang('no') . "</button> \" rel='popover'>
        ...
        if(!empty($_POST)){
            if ($this->form_validation->run() == true) {
                $data = [
                    ...
                    'dren'      => $this->input->post('dren'),
                    'order'     => $this->input->post('order'),
                    'type'      => $this->input->post('type'),
                    'systm'     => $this->input->post('systm'),
                    'status'=>'1',
                    'created_at'=> time(),
                    'created_by'=>$this->session->userdata('user_id'),
                ];

                if ($this->school_model->addSchool($data)) {
                    $this->session->set_flashdata('message', lang('school_added'));
            ...
            $this->load->view($this->theme . 'schools/add', $this->data);
        }
    ...
    public function delete_school($id = null)
    {
        ...
        if ($this->school_model->deleteSchool($id)) {
            $this->sma->send_json(['error' => 0, 'msg' => lang('school_deleted')]);
        }
    }
    ...
    public function edit_school($id = null)
    {
        ...
        $this->form_validation->set_rules('name', lang('school_name'), 'required|min_length[3]');
        if ($this->form_validation->run() == true) {
            $data = [
                'address'   => $this->input->post('address'),
                'code'      => $this->input->post('code'),
...
//                'status'    => $this->input->post('status'),
        ...
        if ($this->form_validation->run() == true && $this->school_model->updateSchool($id, $data)) {
            $this->session->set_flashdata('message', lang('school_updated'));
            admin_redirect('schools');
        }
        else {
            $school = $this->school_model->getSchoolByID($id);

            $this->data['school'] = $school;
            $this->data['id']           = $id;
            $this->data['modal_js']     = $this->site->modal_js();
            $this->data['error']        = validation_errors();
            $this->load->view($this->theme . 'schools/edit', $this->data);
        }
    }
    ...
    public function classes($id = null){
        ...
        $this->data['classes'] =  $this->db->where_in('teaching_type',$cycles)->get('classes')->result();
        $this->data['school'] = $school;
        $this->load->view($this->theme . 'schools/classes', $this->data);
    }
    ...
    public function updates()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            admin_redirect('schools/updates');
        } else {
            ...
            $this->page_construct('schools/updates', $meta, $this->data);
        }
    }
        ...
        if ($this->form_validation->run() == true) {
            if (isset($_FILES['csv_file'])) {
                ...
                foreach ($arrResult as $key => $value) {
                    $school = [
                        'address'      => isset($value[0]) ? trim(strtolower($value[0])) : '',
                        'code'          => isset($value[1]) ? trim(strtolower($value[1])) : '',
                        'name'          => isset($value[2]) ? trim(strtolower($value[2])) : '',
                        'cycle'         => isset($value[3]) ? trim(strtolower($value[3])) : '',
                        'phone'         => isset($value[4]) ? trim(strtolower($value[4])) : '',
                        'dren'          => isset($value[5]) ? trim(strtolower($value[5])) : '',
                        'order'         => isset($value[6]) ? trim(strtolower($value[6])) : '',
                        'type'          => isset($value[7]) ? trim(strtolower($value[7])) : '',
                        'systm'         => isset($value[8]) ? trim(strtolower($value[8])) : '',
                    ];

                    switch ($school['cycle']){
                        case 'prescolaire'          : $school['cycle'] = 'preschool1';break;
                        case 'maternelle'           : $school['cycle'] = 'preschool2';break;
                        case 'primaire'             : $school['cycle'] = 'primary';break;
                        case 'secondaire'           : $school['cycle'] = 'secondary';break;
                        case 'primaire-secondaire'  : $school['cycle'] = 'primary_secondary';break;
                        case 'prescolaire-primaire' : $school['cycle'] = 'preschool1_primary';break;
                    }

                    if (empty($school['code']) || empty($school['phone'])) {
                        $this->session->set_flashdata('error', lang('code') . ', ' . lang('phone') .' ' . lang('required') . ' (' . lang('line_no') . ' ' . $rw . ')');
                        admin_redirect('schools');
                    }
                    else {
                        if ($school_details = $this->school_model->getSchoolByCode($school['code']))
                        {
                            $updated .= '<p>' . lang('Ecole mis a jour') . ' (' . $school['code'] . ')</p>';
                            $this->school_model->updateSchool($school_details->id, $school);
                        }

                        else {
                            $data[] = $school;

                            $this->add_by_csv($school);
                        }
                        $rw++;
                    }
                }
        ...
        if ($this->form_validation->run() == true && !empty($data)) {
            ...
            if (isset($data) && empty($data)) {
                if ($updated) {
                    ...
                    $this->session->set_flashdata('warning', lang('data_x_schools'));
                }
                admin_redirect('schools');
            }
            ...
            $this->data['error']    = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'schools/import', $this->data);
        }
    }

    public function add_by_csv($scv_data = [])
    {

        if (!empty($scv_data)) {
            $data = [
                'address'      => $scv_data['address'],
                'code'          => $scv_data['code'],
                'name'          => $scv_data['name'],
                'cycle'         => $scv_data['cycle'],
                'phone'         => $scv_data['phone'],
                'dren'          => $scv_data['dren'],
                'order'         => $scv_data['order'],
                'type'          => $scv_data['type'],
                'systm'         => $scv_data['systm'],
                'status'        => '1',
                'created_at'    => time(),
                'created_by'    => $this->session->userdata('user_id'),
            ];

            $this->school_model->addSchool($data);
        }


    }
//----------------------------------------------------------------------------------------------------------------------
    public function cart_builder($school_id = '' , $class_id = '' , $sid = '')
    {
        if(!empty($school_id)  and !empty($class_id)){
            $this->session->set_userdata('opened_school',$school_id);
            $this->session->set_userdata('opened_class',$class_id);
        }
        if(!empty($this->session->userdata('opened_school') and empty($school_id))){
            $school_id = $this->session->userdata('school_id');
        }
        if(!empty($this->session->userdata('opened_class')) and empty($class_id)){
            $class_id = $this->session->userdata('opened_class');
        }
        ...
        if (!$this->pos_settings->default_biller || !$this->pos_settings->default_customer || !$this->pos_settings->default_category)
        {
            ...
            admin_redirect('schools/settings');
        }
        if ($register = $this->pos_model->registerData($this->session->userdata('user_id')))
        {
            ...
            admin_redirect('schools/open_register');
        }
...
// 'reference_no' => $this->site->getReference('pay'),
        ...
        if ($this->form_validation->run() == true && !empty($products) && !empty($data)) {
            if ($suspend) {
                //save cart_builder items
                if ($this->pos_model->suspendCartBuilding($data, $products, $did)) {
                    $this->session->set_userdata('remove_posls', 1);
                    $this->session->set_flashdata('message', $this->lang->line('cart_building_saved'));

                    if(!empty($sid)){
                        $redirect_to = 'schools/view/'.$sid;
                    }
                    else{
                        $sus_id = $this->db->order_by('id','DESC')->get_where('suspended_bills',['school_id'=>$school_id,'class_id'=>$class_id])->row()->id;
                        $redirect_to = 'schools/view/'.$sus_id;
                    }
                ...
                if ($sale = $this->pos_model->addSale($data, $products, $payment, $did)) {
                    ...
                    $redirect_to = $this->pos_settings->after_sale_page ? 'pos' : 'schools/view/' . $sale['sale_id'];
            ...
            if ($sid) {
                if ($suspended_sale = $this->pos_model->getOpenBillByID($sid)) {
                    ...
                    $this->data['message']        = lang('school_stuff_loaded');
            ...
            $this->load->view($this->theme . 'schools/cart_builder', $this->data);
        }
    }
...
//$this->sma->print_arrays($payment);
        ...
        if ($this->form_validation->run() == true && $msg = $this->pos_model->addPayment($payment, $customer_id)) {
            ...
            $this->load->view($this->theme . 'schools/cart_builder/add_payment', $this->data);
        }
    }
    ...
    public function add_printer()
    {
        ...
        if ($this->form_validation->run() == true && $cid = $this->pos_model->addPrinter($data)) {
            ...
            admin_redirect('schools/printers');
        } else {
            ...
            $bc                       = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('pos'), 'page' => lang('pos')], ['link' => admin_url('schools/printers'), 'page' => lang('printers')], ['link' => '#', 'page' => lang('add_printer')]];
            $meta                     = ['page_title' => lang('add_printer'), 'bc' => $bc];
            $this->page_construct('schools/add_printer', $meta, $this->data);
        }
    }
    ...
    public function ajaxproducts($category_id = null, $brand_id = null)
    {
        ...
        $config                  = [];
        $config['base_url']      = base_url() . 'schools/ajaxproducts';
    ...
    public function close_register($user_id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->pos_model->closeRegister($rid, $user_id, $data)) {
            ...
            $this->data['modal_js']        = $this->site->modal_js();
            $this->load->view($this->theme . 'schools/cart_builder/close_register', $this->data);
        }
    }
    ...
    public function edit_printer($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->pos_model->updatePrinter($id, $data)) {
            ...
            $bc                       = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('pos'), 'page' => lang('pos')], ['link' => admin_url('schools/printers'), 'page' => lang('printers')], ['link' => '#', 'page' => lang('edit_printer')]];
            $meta                     = ['page_title' => lang('edit_printer'), 'bc' => $bc];
            $this->page_construct('schools/edit_printer', $meta, $this->data);
        }
    }
    ...
    public function email_receipt($sale_id = null, $view = null)
    {
        ...
        $receipt = $this->load->view($this->theme . 'schools/cart_builder/email_receipt', $this->data, true);
        if ($view) {
            echo $receipt;
            die();
        }
    ...
    public function get_printers()
    {
        ...
        $this->datatables
            ...
            ->from('printers')
            ->add_column('Actions', "<div class='text-center'> <a href='" . admin_url('schools/edit_printer/$1') . "' class='btn-warning btn-xs tip' title='" . lang('edit_printer') . "'><i class='fa fa-edit'></i></a> <a href='#' class='btn-danger btn-xs tip po' title='<b>" . lang('delete_printer') . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('schools/delete_printer/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", 'id')
            ->unset_column('id');
        echo $this->datatables->generate();
    }
...
// }
    ...
    public function getSales($warehouse_id = null)
    {
        ...
        $duplicate_link    = anchor('admin/schools/?duplicate=$1', '<i class="fa fa-plus-square"></i> ' . lang('duplicate_sale'), 'class="duplicate_pos"');
        $detail_link       = anchor('admin/schools/view/$1', '<i class="fa fa-file-text-o"></i> ' . lang('view_receipt'));
        ...
        $add_payment_link  = anchor('admin/schools/add_payment/$1', '<i class="fa fa-money"></i> ' . lang('add_payment'), 'data-toggle="modal" data-target="#myModal"');
    ...
    public function open_register()
    {
        ...
        if ($this->form_validation->run() == true && $this->pos_model->openRegister($data)) {
            ...
            $this->page_construct('schools/open_register', $meta, $this->data);
        }
    }
    ...
    public function opened_bills($per_page = 0)
    {
        ...
        $this->data['school_id'] = $this->session->userdata('opened_school');
        ...
        $school = $this->school_model->getSchoolByID($this->session->userdata('opened_school'));
        $class = $this->db->get_where('classes',['id'=>$this->session->userdata('opened_class')])->row();

        $this->pagination->initialize($config);
        $data['r'] = true;
        $bills     = $this->pos_model->fetch_school_bills($config['per_page'], $per_page);
        if (!empty($bills)) {
            $html = '';
            $html .= '<ul class="ob">';
            foreach ($bills as $bill) {
                $html .= '<li>
    <button style="text-align: left" type="button" class="btn sus_sale" id="' . $bill->id . '">
    <h4>' . character_limiter($school->school_name , 15). '</h4>
        <strong>' . $class->level . ' : '.$bill->suspend_note.'</strong>
        <br><span class="label label-primary" style="width: 55px; margin-right:5px ; text-align: left;">'. lang('date') . '</span> : ' . date('d-m-Y H:i:s' , strtotime($bill->date)) . '
        <br><span class="label label-primary" style="width: 55px; margin-right:5px ; text-align: left;">'. lang('items') . '</span> : ' . $bill->count . '
        <br><span class="label label-primary" style="width: 55px; margin-right:5px ; text-align: left;">'. lang('total') . '</span> : ' . $this->sma->formatMoney($bill->total) . '
    </button>
</li>';
            }
            $html .= '</ul>';
    ...
    public function printers()
    {
        ...
        $this->page_construct('schools/printers', $meta, $this->data);
    }
    ...
    public function register_details()
    {
        ...
        $this->load->view($this->theme . 'schools/cart_builder/register_details', $this->data);
    }
    ...
    public function registers()
    {
        ...
        $this->page_construct('schools/registers', $meta, $this->data);
    }
...
//$this->data['customer'] = $this->pos_model->getCompanyByID($this->pos_settings->default_customer);
            ...
            $this->page_construct('schools/settings', $meta, $this->data);
        }
    }
    ...
    public function today_sale()
    {
        ...
        $this->load->view($this->theme . 'schools/cart_builder/today_sale', $this->data);
    }

    /* ------------------------------------------------------------------------------------ */

    public function view($sus_id = null, $modal = false)
    {
        $this->sma->checkPermissions('index');
        if ($this->input->get('id')) {
        ...
        $this->data['error']   = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['message'] = $this->session->flashdata('message');
        $suspended_sale = $this->pos_model->getOpenBillByID($sus_id);
        $inv                   = $suspended_sale;
        ...
        $this->data['rows']            = $this->pos_model->getSuspendedSaleItems($sus_id);
        $biller_id                     = $inv->biller_id;
        $customer_id                   = $inv->customer_id;
        $this->data['biller']          = $this->pos_model->getCompanyByID($biller_id);
        $this->data['customer']        = $this->pos_model->getCompanyByID($customer_id);
        $this->data['payments']        = $this->pos_model->getInvoicePayments($sus_id);
        $this->data['pos']             = $this->pos_model->getSetting();
        $this->data['barcode']         = $this->barcode($inv->reference_no, 'code128', 30);
        $this->data['return_sale']     = $inv->return_id ? $this->pos_model->getInvoiceByID($inv->return_id) : null;
        $this->data['return_rows']     = $inv->return_id ? $this->pos_model->getAllInvoiceItems($inv->return_id) : null;
        $this->data['return_payments'] = $this->data['return_sale'] ? $this->pos_model->getInvoicePayments($this->data['return_sale']->id) : null;
        $this->data['inv']             = $suspended_sale;
    ...
    public function view_bill()
    {
        $this->sma->checkPermissions('index');
        $this->data['tax_rates'] = $this->site->getAllTaxRates();
        $this->load->view($this->theme . 'schools/cart_builder/view_bill', $this->data);
    }

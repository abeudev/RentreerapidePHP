...
class Reports extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->is_fournisseur = $this->sma->in_group('fournisseurs');
    }
    ...
    public function best_sellers($warehouse_id = null)
    {
        ...
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $y1                  = date('Y', strtotime('-0 month'));
        $m1                  = date('m', strtotime('-0 month'));
        ...
        $m4sdate             = $y4 . '-' . $m4s . '-01 23:59:59';
        $m3edate             = $y4 . '-' . $m4e . '-' . days_in_month($m4e, $y4) . ' 23:59:59';

        $this->data['m4']    = date('M Y', strtotime($y4 . '-' . $m4s)) . ' - ' . date('m Y', strtotime($y4 . '-' . $m4e));
        $this->data['m4bs']  = $this->reports_model->getBestSeller($m4sdate, $m3edate, $warehouse_id);
        ...
        if ($pdf || $xls) {
            ...
            if ($warehouse_id) {
                $this->datatables
                    ...
                    ->from('products')
                    ->join("( SELECT * from {$this->db->dbprefix('warehouses_products')} WHERE warehouse_id = {$warehouse_id}) wp", 'products.id=wp.product_id')
    ...
    {
        $has_permission = ($this->Owner || $this->is_fournisseur);
        if (!$has_permission) {
    ...
    public function profit($date = null, $warehouse_id = null, $re = null)
    {
        ...
        $this->data['sales']      = $this->reports_model->getDailySale($date, $warehouse_id);
        $this->data['warehouses'] = $this->site->getAllWarehouses();
        $this->data['swh']        = $warehouse_id;
        $this->data['date']       = $date;
        if ($re) {
    ...
    public function profit_loss($start_date = null, $end_date = null)
    {
        ...
        $this->data['total_received_om']    = $this->reports_model->getTotalReceivedPaymentAmount($start, $end,'OM');
        $this->data['total_received_momo']    = $this->reports_model->getTotalReceivedPaymentAmount($start, $end,'MOMO');

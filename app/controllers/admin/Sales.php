...
class Sales extends MY_Controller
{
    ...
    public function getSales($warehouse_id = null)
    {
        ...
        $update_sale_link  = anchor('admin/sales/update_status/$1', '<i class="fa fa-check-square-o"></i> ' . lang('sale_status'), 'data-toggle="modal" data-target="#myModal"');
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("{$this->db->dbprefix('sales')}.id as id, DATE_FORMAT({$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date, reference_no, biller, sale_status, grand_total, paid, (grand_total-paid) as balance, payment_status,payment_method,  {$this->db->dbprefix('sales')}.attachment, return_id")
                ->from('sales')
        ...
        $this->datatables->add_column('Test', '$1', 'id');
        $this->datatables->add_column('Actions', $action, 'id');
        echo $this->datatables->generate();
    }

    public function getSalesss($warehouse_id = null)
    {
        $this->sma->checkPermissions('index');
    ...
    public function suggestions($pos = 0)
    {
        ...
        if ($rows) {
            ...
            foreach ($rows as $row) {
                ...
                $pr[] = ['id' => sha1($c . $r), 'item_id' => $row->id, 'label' => $row->name . ' (' . $row->warehouse_name . ') '.$this->sma->formatMoney($row->price), 'category' => $row->category_id,
                    'row'     => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options, ];
                $r++;
            }

...
class Main extends MY_Shop_Controller
{
    ...
    public function index()
    {
        ...
        if(!empty($this->session->userdata('opened_lib'))){
            $where = " where p.warehouse = {$this->session->userdata('opened_lib')} and p.quantity > 0";
        }

        $this->data['standart_products']        = [];
        $this->data['product_categories']       = $this->db->query('select DISTINCT(category_id)  , c.name ,c.slug   from sma_products as p join sma_categories as c on c.id = p.category_id'.$where)->result();
        $this->data['product_categories_count'] = $this->db->query("select count(category_id) as total_items , c.name from sma_products as p join sma_categories as c on c.id = p.category_id {$where} GROUP BY p.category_id")->result();

        $slider = $this->shop_settings->slider;
        if(!empty($this->session->userdata('opened_lib'))){
            $slides = $this->db->get_where('supplier_slides',['warehouse_id'=>$this->session->userdata('opened_lib')]);
        ...
        foreach ($this->data['product_categories'] as $category){
            $category_products = $this->shop_model->getStandartProducts2(['c.id'=>$category->category_id] , 6);
            foreach ($category_products as $product){
                $this->data['standart_products'][] = $product;
                $fetched_product_ids[] = $product->id;
            }
        }

        $this->data['fetched_ids'] = $fetched_product_ids;

        $this->data['slider']            = json_decode($slider);
        $this->data['page_title']        = $this->shop_settings->shop_name;
        $this->data['page_desc']         = $this->shop_settings->description;
        $date = date('Y-m-d H:i:s',time());
        $this->data['week_deals'] = $this->db->select("week_deals.* ,p.name , p.price, p.image , p.category_id , p.warehouse_name , c.name as category_name")->where(['ending_date >'=>$date])->join('products as p', 'p.id = week_deals.product_id')->join('categories as c','c.id = p.category_id')->get('week_deals')->result();
        $this->data['featured_products']        = $this->shop_model->getFeaturedProducts((!empty($this->data['week_deals'])?'8':'16'));

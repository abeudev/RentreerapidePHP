...
class Shop_model extends CI_Model
{
    ...
    public function addSale($data, $items, $customer, $address)
    {
        ...
        if (is_array($customer) && !empty($customer)) {
            ...
            if(!$this->db->get_where('companies', ['id'=>$customer['id']])->num_rows() > 0){
                $this->db->insert('companies', $customer);
                $data['customer_id'] = $this->db->insert_id();
//                echo 'data added to compnaies <br>';
            }
        ...
        if (is_array($address) && !empty($address)) {
            ...
            //check the existance of the address
            if(!$this->db->get_where('addresses', ['id'=>$address['id']])->num_rows() > 0){
...
//            echo    'successfully added to sale';
    ...
    public function getAddressByID($id , $return_mode = 'row')
    {
        $q = $this->db->get_where('addresses', ['id' => $id], 1);
        switch($return_mode){
            case 'row': return $q->row(); break;
            case 'result' : return $q->result(); break;
            case 'result_array' : return $q->result_array(); break;
            case 'row_array' : return $q->row_array(); break;

            default : return $q->row(); break;
        }
    ...
    {
        return $this->db->select('addresses.* , vl.region_name as ville , vl.id as region_id , reg.region_name')
                        ->join('shipping_regions as vl','vl.id = addresses.region_id')
                        ->join('shipping_regions as reg','reg.id = addresses.state')
                        ->get_where('addresses', ['company_id' => $this->session->userdata('company_id')])->result();
    }
    ...
    public function getAllCategories()
    {
        ...
        if(!empty($this->session->userdata('opened_lib'))){
            $where = " AND  {$this->db->dbprefix('products')}.warehouse = {$this->session->userdata('opened_lib')}";
        }
        if ($this->shop_settings->hide0) {
            $pc = "(SELECT count(*) FROM {$this->db->dbprefix('products')} WHERE {$this->db->dbprefix('products')}.category_id = {$this->db->dbprefix('categories')}.id {$where}) ";
    ...
    public function getAllWarehouseWithPQ($product_id, $warehouse_id = null)
    {
        ...
        $this->db->select('' . $this->db->dbprefix('warehouses') . '.*, ' . $this->db->dbprefix('warehouses_products') . '.quantity, ' . $this->db->dbprefix('warehouses_products') . '.rack')
            ...
            ->group_by('warehouses.id , sma_warehouses_products.quantity , sma_warehouses_products.rack');
    ...
    {
        if(!empty($this->session->userdata('opened_lib'))){
            $this->db->where(['products.warehouse'=>$this->session->userdata('opened_lib')]);
        }
        $this->db->select("{$this->db->dbprefix('products')}.id as id, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.slug as slug, {$this->db->dbprefix('products')}.price, warehouse_name, quantity, type, promotion, promo_price, start_date, end_date, b.name as brand_name, b.slug as brand_slug, c.name as category_name, c.slug as category_slug")
            ->join('brands b', 'products.brand=b.id', 'left')
            ->join('categories c', 'products.category_id=c.id', 'left')
            ->where('products.quantity > products.alert_quantity')
    ...
    public function getStandartProducts($limit = 100, $promo = true)
    {
        if(!empty($this->session->userdata('opened_lib'))){
            $this->db->where(['products.warehouse'=>$this->session->userdata('opened_lib')]);
        }
        $this->db->select("{$this->db->dbprefix('products')}.id as id, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.slug as slug, {$this->db->dbprefix('products')}.price,warehouse_name, quantity, type, promotion, promo_price, start_date, end_date, b.name as brand_name, b.slug as brand_slug, c.name as category_name, c.slug as category_slug , featured")
            ->join('brands b', 'products.brand=b.id', 'left')
            ->join('categories c', 'products.category_id=c.id', 'left');
//            ->where('products.featured !=', 1);
//            ->where('hide ', 1);
...
//        }
    ...
    public function getStandartProducts2($where = [] , $limit = 16, $promo = true)
    {
        if(!empty($where)){
            $this->db->where($where);
        }
...
//        }
        ...
        $this->db->order_by("{$this->db->dbprefix('products')}.id",'RANDOM');
        $this->db->order_by("{$this->db->dbprefix('products')}.id",'DESC');
        $this->db->limit($limit);
        return $this->db->get('products')->result();
    }

    public function getProducts3($where = [] , $like = [])
    {
        if(!empty($where)){
            $this->db->where($where);
        }
        ...
        if(!empty($like)){
            if(!empty($like[0]) and !empty($like[1])){
                $option = (!empty($like[2]))?$like[2] : 'both';
                $this->db->like($like[0] , $like[1] ,$option);
            }
            else{
                $this->db->like($like);
            }
        ...
        $sp = $this->getSpecialPrice();
        $this->db->order_by("{$this->db->dbprefix('products')}.price",'ASC');
        $this->db->order_by("{$this->db->dbprefix('products')}.promo_price",'ASC');
        return $this->db->get('products')->result();
    }

    public function getWareHouseIdByProductId($product_id){
        $p = $this->db->get_where('products',['id'=>$product_id]);
        if($p->num_rows() > 0){
            return $p->row()->warehouse;
        }
        else{
            return 0;
        }
    ...
    public function getOtherProducts($id, $category_id, $brand , $limit = 18)
    {
        $this->db->select("{$this->db->dbprefix('products')}.id as id, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.slug as slug, {$this->db->dbprefix('products')}.price, warehouse_name, quantity, type, promotion, promo_price, start_date, end_date, b.name as brand_name, b.slug as brand_slug, c.name as category_name, c.slug as category_slug")
            ->join('brands b', 'products.brand=b.id', 'left')
            ->join('categories c', 'products.category_id=c.id', 'left')
    ...
    public function getProductDetails($slug)
    {
        ...
        $this->db->select("{$p}.id , {$p}.name , {$p}.code , {$p}.quantity as stock , {$p}.views , {$p}.hsn_code , {$p}.warehouse_name , {$p}.category_id, {$p}.subcategory_id , {$p}.unit, {$p}.brand, {$p}.slug , {$p}.type , {$p}.file, {$p}.details,{$p}.image,{$p}.name , {$p}.promo_price , {$p}.price");
        $sp = $this->getSpecialPrice();
        if ($sp->cgp) {
            $this->db->select('cgp.price as special_price', false)->join($sp->cgp, 'products.id=cgp.product_id', 'left');
        } elseif ($sp->wgp) {
    ...
    public function getProductOptionsWithWH($product_id, $warehouse_id = null)
    {
        ...
        $this->db->select($this->db->dbprefix('product_variants') . '.*, ' . $this->db->dbprefix('warehouses') . '.name as wh_name, ' . $this->db->dbprefix('warehouses') . '.id as warehouse_id, ' . $this->db->dbprefix('warehouses_products_variants') . '.quantity as wh_qty')
            ...
            ->group_by(['' . $this->db->dbprefix('product_variants') . '.id', '' . $this->db->dbprefix('warehouses_products_variants') . '.warehouse_id','sma_warehouses_products_variants.quantity'])
    ...
    {
        $sp = $this->getSpecialPrice();
        $which = ($sp->cgp) ? 'cgp.price' : 'wgp.price';

        $where_warehouse = [];
        if($this->shop_settings->warehouse != '0'){$where_warehouse  = ['warehouses_products.warehouse_id'=>$this->shop_settings->warehouse];}
        else{
            if(!empty($this->session->userdata('opened_lib'))){$where_warehouse = ['warehouses_products.warehouse_id'=>$this->session->userdata('opened_lib')];}
        }

        $this->db->select("{$this->db->dbprefix('products')}.id as id, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.slug as slug, {$this->db->dbprefix('products')}.price, {$this->db->dbprefix('warehouses_products')}.quantity as quantity, warehouse_name, type, promotion, promo_price, start_date, end_date, product_details as details")
            ->from('products')
            ->join('warehouses_products', 'products.id=warehouses_products.product_id', 'left')
            ->where($where_warehouse)
            ->where('products.quantity > products.alert_quantity')
            ->group_by('products.id ,  sma_warehouses_products.quantity');
//            ->group_by('products.id ,  sma_warehouses_products.quantity , '.$which);
        ...
        if (!empty($filters)) {
            ...
            if (!empty($filters['min_price'])) {
                $this->db->where('sma_products.price >=', $filters['min_price']);
            }
            if (!empty($filters['max_price'])) {
                $this->db->where('sma_products.price <=', $filters['max_price']);
            }
    ...
    public function getProducts2($filters = [])
    {
        ...
        $this->db->select("{$this->db->dbprefix('products')}.id as id, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.slug as slug, {$this->db->dbprefix('products')}.price, {$this->db->dbprefix('warehouses_products')}.quantity as quantity, warehouse_name, type, promotion, promo_price, start_date, end_date, product_details as details,b.name as brand_name, b.slug as brand_slug, c.name as category_name, c.slug as category_slug")
            ->from('products')
            ->join('warehouses_products', 'products.id=warehouses_products.product_id', 'left')
            ->join('brands b', 'products.brand=b.id', 'left')
            ->join('categories c', 'products.category_id=c.id', 'left')
...
//            ->group_by('products.id ,  sma_warehouses_products.quantity , '.$which);
        ...
        $this->db->where('hide !=', 1)
            ->limit($filters['limit'], $filters['offset']);
        if (!empty($filters)) {
            if(!empty($filters['where_not_in']) and !empty($filters['where_not_in_key'])){
                $this->db->where_not_in($filters['where_not_in_key'] , $filters['where_not_in']);
            }
            if (!empty($filters['promo'])) {
                $today = date('Y-m-d');
                $this->db->where('promotion', 1)->where('start_date <=', $today)->where('end_date >=', $today);
...
//            ->group_by('products.id , '.$which);
    ...
    public function getProductsCount2($filters = [])
    {
        $sp = $this->getSpecialPrice();
        $which = ($sp->cgp) ? 'cgp.price' : 'wgp.price';
        $where_warehouse = [];
...
//            ->group_by('products.id , '.$which);
    ...
    public function getProductVariants($product_id, $warehouse_id = null, $all = null)
    {
        ...
        $this->db->select('product_variants.id as id, product_variants.name as name, product_variants.price as price, product_variants.quantity as total_quantity, FWPV.quantity as quantity', false)
            ...
            ->group_by('product_variants.id, FWPV.quantity');
    ...
    public function fetch_school_bills($limit, $start , $school_id = '' , $class_id = '')
    {
        $this->db->limit($limit, $start);
        $this->db->order_by('id', 'asc');
        $this->db->where(['school_id'=>$school_id , 'class_id'=>$class_id]);
    ...
    {
        $this->db->select('' . $this->db->dbprefix('products') . '.id, code, warehouse_name,price, image, ' . $this->db->dbprefix('products') . '.name as name')

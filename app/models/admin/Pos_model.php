...
class Pos_model extends CI_Model
{
    ...
    public function fetch_bills($limit, $start)
    {
        ...
        $this->db->where(['school_id'=>null , 'class_id'=>null]);
    ...
    public function fetch_school_bills($limit, $start)
    {
        if (!$this->Owner && !$this->Admin) {
            $this->db->where('created_by', $this->session->userdata('user_id'));
        }
        $this->db->limit($limit, $start);
        $this->db->order_by('id', 'asc');
        $this->db->where(['school_id'=>$this->session->userdata('opened_school') , 'class_id'=>$this->session->userdata('opened_class')]);
        $query = $this->db->get('suspended_bills');
    ...
    public function suspendCartBuilding($data = [], $items = [], $did = null)
    {
        $sData = [
            'count'             => $data['total_items'],
            'biller_id'         => $data['biller_id'],

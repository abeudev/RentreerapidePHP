...
class Products_model extends CI_Model
{
    ...
    public function addProduct($data, $items, $warehouse_qty, $product_attributes, $photos)
    {
        if ($this->db->insert('products', $data)) {
            $product_id = $this->db->insert_id();
            $this->db->insert('product_shipping_group',['product_id'=>$product_id , 'group_id'=>$_POST['product_group']]);
            ...
            if(!empty($data['quantity'])){
               $this->db->where(['id'=>$product_id])->update('products',['quantity'=>$data['quantity']]);
               $this->db->where(['product_id'=>$product_id])->update('warehouses_products',['quantity'=>$data['quantity']]);
            }
            return true;
        }
        return false;
    ...
    public function deleteProduct($id)
    {
        if($this->session->userdata('group_id') != '1'){
            $this->db->where(['products.supplier1'=>$this->session->userdata('company_id')]);
        }
    ...
    public function updateProduct($id, $data, $items, $warehouse_qty, $product_attributes, $photos, $update_variants)
    {
        if ($this->db->update('products', $data, ['id' => $id])) {
            ...
            if($this->db->get_where('product_shipping_group',['product_id'=>$id])->num_rows() > 0){
                $this->db->where(['product_id'=>$id])->update('product_shipping_group',['group_id'=>$_POST['product_group']]);
            }
            else{
                $this->db->insert('product_shipping_group',['group_id'=>$_POST['product_group'],'product_id'=>$id]);
            }

...
class Reports_model extends CI_Model
{
    ...
    public function getBestSellers($start_date, $end_date, $warehouse_id = null)
    {
        $this->db
            ->select('product_names, product_code')->select_sum('quantity')
            ->join('sales', 'sales.id = sale_items.sale_id', 'left')
            ->where('date >=', $start_date)->where('date <=', $end_date)
            ->group_by('product_name, product_code')->order_by('sum(quantity)', 'desc')->limit(10);
        if ($warehouse_id) {
    ...
    public function getDailySale($date, $warehouse_id = null)
    {
        ...
        $myQuery = "SELECT SUM(COALESCE(subtotal)) as total , sum(COALESCE(quantity)) as total_product FROM " . $this->db->dbprefix('sale_items') . " join " . $this->db->dbprefix('sales') . " on " . $this->db->dbprefix('sales') . ".id = " . $this->db->dbprefix('sale_items') . ".sale_id WHERE";
        if ($warehouse_id) {
            $myQuery .= " warehouse_id = {$warehouse_id} AND ";
        }
        $myQuery .= " DATE_FORMAT( date,  '%Y-%m-%d' ) =  '{$year}-{$month}-{$day}'
            GROUP BY DATE_FORMAT( date,  '%e' )";
        $q = $this->db->query($myQuery, false);
        if ($q->num_rows() > 0) {
            return $q->row();
    ...
    public function getTotalReceivedPaymentAmount($start, $end , $payment_mode)
    {
        $this->db->select('count(id) as total, SUM(COALESCE(amount, 0)) as total_amount', false)
            ->where('type', 'received')->where('paid_by', $payment_mode)
            ->where('date BETWEEN ' . $start . ' and ' . $end);
        $q = $this->db->get('payments');
        if ($q->num_rows() > 0) {
            return $q->row();

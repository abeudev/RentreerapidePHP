...
class Sales_model extends CI_Model
{
    ...
    public function addPayment($data = [], $customer_id = null)
    {
        ...
        //check if sale is istallment
        $sale_id = $data['sale_id'];
        $amount = $data['amount'];
        $where_sale_id = ['sale_id'=>$sale_id];
        $installment_param_data = $this->db->get_where('installment_param',$where_sale_id);

        if($installment_param_data->num_rows() > 0)
        {
            $installment_param = $installment_param_data->row();
            $pay_record = $this->db->get_where('installment_pay_record',$where_sale_id)->row();
            $weekly_pay = $installment_param->weekly_pay;
            $num_of_week = $installment_param->num_of_week;

            for($i = 1 ; $i<=(int)$num_of_week ; $i++){
               if($amount > 0){
                   $w = 'w'.$i;
                   $wp = $pay_record->$w;
                   $av_wp = 0;

                   if($wp == null ||  $wp < $weekly_pay){

                       //if value for whe evaluated week is not null and less than weekly payment
                       if($wp != null && $wp > 0 && $wp < $weekly_pay){
                           $to_pay = ($weekly_pay - $wp);
                           if($amount >= $to_pay){
                               $av_wp = $to_pay + $wp;
                           ...
                           if($amount > $weekly_pay){
                               $av_wp = $weekly_pay;
                               $amount = $amount - $weekly_pay;
                           }
                           else{
                               $av_wp = $amount;
                               $amount = 0;
                       ...
                       $this->db->where(['sale_id'=>$sale_id])->update('installment_pay_record',[$w => $av_wp]);
                   }
               }
            }

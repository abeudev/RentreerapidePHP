...
//        $qt = $this->db->get_where('products',['id'=>$product_id])->quantity;

//        if ($real_item_qty > $qt && !$this->Settings->overselling) {
//            $this->session->set_flashdata('error', sprintf(lang('quantity_out_of_stock_for_%s').'aaa', $product_name));
...
//        }
    ...
    {
        return $this->db->get_where('products',['id'=>$product_id])->row()->quantity;
    }
    ...
    public function getCompanyByID($id , $return_mode = 'row')
    {
    ...
    public function getPurchasedItems($product_id, $warehouse_id = null, $option_id = null, $nonPurchased = false)
    {
        $orderby = empty($this->Settings->accounting_method) ? 'asc' : 'desc';
        $this->db->select('id, quantity, quantity_balance, net_unit_cost, unit_cost, item_tax');
        $this->db->where('product_id', $product_id)->where((!empty($warehouse_id)?['warehouse_id'=>$warehouse_id]:[]))->where('quantity_balance !=', 0);

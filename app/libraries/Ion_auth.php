...
class Ion_auth
{
    ...
    public function forgotten_password2($identity)
    {    //changed $email to $identity
        if ($this->auth_model->forgotten_password($identity)) {   //changed
            // Get user information
            $user = $this->where($this->config->item('identity', 'ion_auth'), $identity)->where('active', 1)->users()->row();  //changed to get_user_by_identity from email
            ...
            if ($user) {
                $data = [
                    ...
                    'user_name'           => $user->first_name . ' ' . $user->last_name,
                    'forgotten_password_code' => $user->forgotten_password_code,
                    'reset_password_link' => base_url('reset_password/' . $user->forgotten_password_code),
                    'site_link'           => base_url(),
                    'logo'                => base_url() . 'assets/uploads/logos/' . $this->Settings->logo,
                    'site_name'           => $this->Settings->site_name,
    ...
    public function register($username, $password, $email, $additional_data = [], $active = '1', $notify = false)

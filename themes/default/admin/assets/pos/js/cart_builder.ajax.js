        ...
        if (protect_delete == 1) {
            ...
            bootbox.confirm(lang.r_u_sure, function(result) {
                if (result) {
                    ...
                    $('#modal-loading').show();
                    window.location.href = site.base_url + 'schools/cart_builder/'+school_id+'/'+class_id+'';
                }
            });
        }
    });
...
// var itn = parseInt(Object.keys(sortedItems).length);
        $.each(sortedItems, function() {
            ...
            var product_id = item.row.id,
                ...
                item_supplier = item.row.warehouse_name
...
// item_price = unit_price + parseFloat(this.price) * parseFloat(base_quantity);
            ...
            tr_html =
                ...
                '<div class="sname" id="name_' + row_no + '">' + item_name + (sel_opt != '' ? ' (' + sel_opt + ')' : '') + '   </div>' +
                '<div class="label label-success">'+item_supplier + '</div>' +
                '   <span class="lb"></span>' +
                '</td>';

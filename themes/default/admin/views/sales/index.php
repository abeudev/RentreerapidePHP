...
<style>
    .progress{background-color:#79696982;}
</style>
<?php
    $percentage = [];
    $sales = $this->db->select('id , payment_method , payment_status')->get('sales')->result();
    $installment_orders = $this->db->query('select s.id as order_id , s.grand_total as total , s.payment_method , p.weekly_pay , pr.* from sma_sales as s join sma_installment_pay_record as pr on pr.sale_id = s.id join sma_installment_param as p on p.sale_id = s.id where s.payment_method = \'installment\'')->result_array();

    foreach ($sales as $sale){
        $percentage['sale_'.$sale->id] = $sale->payment_status;

        if($sale->payment_method == 'installment'){
            $percentage['sale_'.$sale->id] = '0';
        }

        foreach ($installment_orders as $io) {
            if($sale->id == $io['order_id'] and $io['payment_method'] == 'installment'){
                $paid_count = 0;
                $total_paid = 0;
                $sale_id = $io['order_id'];
                for ($i = 1; $i <= 12; $i++) {
                ...
                $percentage['sale_'.$sale->id] =  $percentage_paid.'';
            }
        }
    }
...
<script>
    ...
    function row_percentage(x) {
            var percentage = percentages['sale_'+x+''];
            if(is_numeric(percentage)){
                var progress =  '<div class="progress" style="height: 18px; width: 94px">'+
                                '   <div class="progress-bar progress-bar-striped progress-bar-success" role="progressbar" style="width:'+percentage+'%" aria-valuenow="10" aria-valuemin="0" aria-valuemax="100">'+percentage+'%</div>'+
                                '</div>';
                return progress;
            }
            else{
                return '<div class="text-center"><span class="row_status label '+((percentage == 'paid')?'label-success':'label-warning')+'">'+percentage+'</span></div>';
            }
    ...
    $(document).ready(function () {
        oTable = $('#SLData').dataTable({
            ...
            "aoColumns": [
                ...
                null,
                null,
                {"mRender": row_status}, //sale_status
                {"mRender": currencyFormat},
                {"mRender": currencyFormat},
                {"mRender": currencyFormat},
                {"mRender": pay_status},
                {"mRender": payment_method},
                {"bVisible": false},
                {"bVisible": false},
                {"bSortable": false, "mRender":row_percentage},
                {"bSortable": false}
                ],
                ...
                <div class="table-responsive">
                    <table id="SLData" class="table table-bordered table-hover table-striped" cellpadding="0" cellspacing="0" border="0">
                        ...
                        <tr>
                            ...
                            <th><?= lang('Mode de paiment'); ?></th>
                            <th></th>
                            <th></th>
                            <th style="width:200px; text-align:center;"><?= lang('Progression'); ?></th>

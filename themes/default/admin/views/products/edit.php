            ...
            <div class="col-lg-12">
                    ...
                    <div class="form-group">
                        ...
                        <select name="product_group" id="group" class="form-control select2">
                            <?php foreach($product_group as $group):?>
                                <option value="<?=$group->id;?>" <?=($produc_group_selected->group_id == $group->id)?'selected':null;?>><?=strtoupper($group->group_name);?> (<?=$group->description;?>)</option>
                            <?php endforeach;?>
                        </select>
                    </div>
                ...
                <div class="col-md-6 col-md-offset-1">
                    <div class="standard">
                            ...
                            if (!empty($warehouses) || !empty($warehouses_products))
                            {
                                ...
                                if (!empty($warehouses_products))
                                {
                                    foreach ($warehouses_products as $wh_pr) {
                                        if($wh_pr->id == $this->session->userdata('warehouse_id')){
                                            echo '<span class="bold text-info">' . $wh_pr->name . ': <input type="text" value="' . $this->sma->formatDecimal($wh_pr->quantity) . '" id="vwh_qty_' . $wh_pr->id . '" name="quantity"><span class="padding05" id="rwh_qty_' . $wh_pr->id . '">' . $this->sma->formatQuantity($wh_pr->quantity) . '</span>' . ($wh_pr->rack ? ' (<span class="padding05" id="rrack_' . $wh_pr->id . '">' . $wh_pr->rack . '</span>)' : '') . '</span><br>';
                                        }
                                    }
                                }
                        ...
                        <div id="attrs"></div>
                        <div class="well well-sm dis-none">
                            <?php
                            if ($product_options) {
                                ?>

...
<head>
    ...
    <script type="text/javascript">if(parent.frames.length !== 0){top.location = '<?=admin_url('schools')?>';}</script>
    <base href="<?=base_url()?>"/>
...
<div id="wrapper">
    <header id="header" class="navbar">
        <div class="container">
            <a style="width:150px;position:relative;height: auto !important;" class="navbar-brand" href="<?=admin_url()?>"><span class="logo"><span class="pos-logo-lg"><?=$Settings->site_name?></span><span class="pos-logo-sm"><?=lang('schools')?></span></span></a>
            ...
            <div class="header-nav">
                ...
                <ul class="nav navbar-nav">
                    <li class="dropdown">
                        <a class="btn borange pos-tip" title="<?=lang('dashboard')?>" data-placement="bottom" href="<?=admin_url()?>">
                            <i class="fa fa-dashboard"></i>
                            <?=lang('dashboard');?>
                        </a>
                    </li>
                    <li class="dropdown">
                        <a class="btn bblue pos-tip" title="<?=lang('schools')?>" data-placement="bottom" href="<?=admin_url('schools')?>">
                            <i class="fa fa-th"></i>
                            <?=lang('Liste des Etablisements');?>
                        </a>
                    </li>
                </ul>
                ...
                <ul class="nav navbar-nav pull-right">
                    <li class="dropdown">
                        <a class="btn blightOrange pos-tip" id="opened_bills" title="<span><?=lang('school_stuffs')?></span>" data-placement="bottom" data-html="true" href="<?=admin_url('schools/opened_bills')?>" data-toggle="ajax">
                            <i class="fa fa-th"></i>
                            <?=lang('Liste des Fournitures');?>
                        </a>
                    </li>
                </ul>
            </div>
        ...
        <div class="c1">
            <div class="pos">
                ...
                <div id="pos">
                    ...
                    echo admin_form_open('schools/cart_builder/'.$school_id.'/'.$class_id.'/'.$sid, $attrib);?>
                    <div id="leftdiv">
                        ...
                        <div id="left-top">
                            <div class="dis-none" style="position: absolute; <?= $Settings->user_rtl ? 'right:-9999px;' : 'left:-9999px;';?>">
                            ...
                            <div class="no-print">
                                ...
                                ?>
                                <div style="margin-top: 20px;" class="form-group" id="ui">
                                    ...
                                    <?php if ($Owner || $Admin || $GP['products-add']) {
                                    ?>
                                        <div class="input-group-addon dis-none" style="padding: 2px 8px;display: none !important;">
                                ...
                                <div id="botbuttons" class="col-xs-12 text-center">
                                    ...
                                    <?php if(!empty($sid)):;?>
                                        <div class="row">
                                            ...
                                            <div class="col-xs-4" style="padding:0px">
                                                <a href="<?=admin_url('schools/view/'.$sid);?>" target="_blank" type="button" class="btn btn-warning btn-block btn-flat" id="preview"> <?= lang('Voir la liste'); ?> </a>
                                            </div>
                                            <div class="col-xs-4" style="padding: 0;">
                                                <button type="button" class="btn btn-success btn-block btn-flat" id="suspend"> <?= lang('save'); ?> </button>
                                            </div>
                                        </div>
                                    <?php else :; ?>
                                        <div class="row">
                            ...
                            <div class="quick-menu">
                                <div id="proContainer">
                                    <div id="ajaxproducts">
                                        ...
                                        ?>
                                            <div class="btn-group">
                                                <button style="z-index:10003;" class="btn btn-primary pos-tip dis-none" type="button" id="sellGiftCard" title="<?=lang('sell_gift_card')?>">
...
<div class="modal fade in" id="susModal" tabindex="-1" role="dialog" aria-labelledby="susModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p><?=lang('type_comment_save');?></p>

                <div class="form-group">
                    <?=lang('reference_note', 'reference_note');?>
                    <?= form_input('reference_note', (!empty($reference_note) ? $reference_note : 'Founiture Scolaire'), 'class="form-control kb-text" id="reference_note"'); ?>
                </div>
...
//                    $(this).val('');
            ...
            select: function (event, ui) {
                ...
                if (ui.item.id !== 0) {
                    ...
                    $.ajax({
                        type: "get",
                        url: "<?=admin_url('schools/getProductPromo');?>",
...
//                    bootbox.alert('<?//=lang('no_match_found')?>//');
        ...
        $(document).on('click', '.product', function (e) {
            ...
            $.ajax({
                type: "get",
                url: "<?=admin_url('schools/getProductDataByCode')?>",
...
//                        bootbox.alert('<?//=lang('no_match_found')?>//');
        ...
        $(document).on('click', '.category', function () {
            if (cat_id != $(this).val()) {
                ...
                $.ajax({
                    type: "get",
                    url: "<?=admin_url('schools/ajaxcategorydata');?>",
        ...
        $(document).on('click', '.brand', function () {
            if (brand_id != $(this).val()) {
                ...
                $.ajax({
                    type: "get",
                    url: "<?=admin_url('schools/ajaxbranddata');?>",
        ...
        $(document).on('click', '.subcategory', function () {
            if (sub_cat_id != $(this).val()) {
                ...
                $.ajax({
                    type: "get",
                    url: "<?=admin_url('schools/ajaxproducts');?>",
...
<script type="text/javascript" src="<?=$assets?>pos/js/cart_builder.ajax.js"></script>
<?php //include_once('pos_ajax_debug.php'); ?>
<?php
    ...
    <script type="text/javascript">
        ...
        function printOrder() {
            $.each(order_printers, function() {
                ...
                'text': order_data };
                $.get('<?= admin_url('schools/p/order'); ?>', {data: JSON.stringify(socket_data)});
            });
            return false;
        }
        ...
        function printBill() {
            ...
            $.get('<?= admin_url('schools/p'); ?>', {data: JSON.stringify(socket_data)});
            return false;
        }
    </script>
    <?php
...
//    /* include FCPATH.'themes'.DIRECTORY_SEPARATOR.$Settings->theme.DIRECTORY_SEPARATOR.'views'.DIRECTORY_SEPARATOR.'schools'.DIRECTORY_SEPARATOR.'remote_printing.php'; */
